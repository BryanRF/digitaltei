    ...
    <div class="grid grid-cols-6">
        <h2 class="col-span-6 md:col-span-3 my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            ...
            <a href="{{route('employee.index')}}"
                class=" items-center mt-3 justify-between px-4 py-2 text-sm 
                font-semibold leading-5 text-white transition-colors duration-150
        ...
        { 
            render: function (data, type, row, meta) {
                return  '<td class="px-4 py-3 text-sm">'+
                            '<button class="px-2 py-1 text-xs font-semibold leading-tight hover:button active:button button rounded " onclick="copyText(this.innerText)" title="Copiar">'+row.document+'</button>'+
                        '</td>';
            },
        },
        { 
            render: function (data, type, row, meta) {
                return  '<td class="px-4 py-3 text-xs">'+
                           ' <button class="px-2 py-1 text-xs font-semibold leading-tight hover:button active:button button rounded " onclick="copyText(this.innerText)" title="Copiar">'+row.email+'</button>'+
                        '</td>';
            },
        },
        { 
            render: function (data, type, row, meta) {
                return  '<td class="px-4 py-3 text-sm">'+
                            '<button class="px-2 py-1 text-xs font-semibold leading-tight hover:button active:button button rounded " onclick="copyText(this.innerText)" title="Copiar">'+row.phone+'</button>'+
                        '</td>';
            },
        },
            ...
            render: function (data, type, row, meta) {
                ...
                if (row.file != null) {
                    if (row.file.indexOf('.pdf') !== -1) {
                        html= '<a href="{{ Storage::url(":archivefile") }}" class="hover:button active:button button text-xs hover:bg-red-600 text-white font-bold p-2 text-center rounded" download>' +
                               '<i class="fas fa-download"></i> PDF' +
                               '</a>';
                    } else if (row.file.indexOf('.doc') !== -1) {
                        html= '<a href="{{ Storage::url(":archivefile") }}" class="hover:button active:button button text-xs hover:bg-blue-600 text-white font-bold p-2 text-center rounded" download>' +
                               '<i class="fas fa-download"></i> Word' +
                               '</a>';
                    }
                } else {
                    html= '<button disabled class="text-xs  button p-2 text-center rounded" title="Sin archivo">' +
